...
The final `PhaseEstimate` assembly is also shared (not hand-rolled per twin): `build_fn` calls `assemble_phase_estimate(*, phase, twin_name, algorithm, point_mid, ai_mid, manual_mc, ai_mc, roster, inputs, breakdown, effective_reduction, assumption_impact_factor, notes)` in `_twin_base.py` — it runs `result_to_hour_range` on both `MCResult`s, `attribute_roles` for both buckets, and maps `inputs.risks` via `risks_from_inputs`; the twin passes `risk_specs_from(inputs.risks)` into `propagate_phase`. Twins differ only in `phase`/`twin_name`/`algorithm`, the per-twin `assumption_impact_factor` (development & qa `0.05`, others `0.1`), and their `notes` — preserve those per-twin when touching a twin.
...
- **AI-effectiveness** — `_twin_base.make_reduction_sampler(...)` builds the per-draw sampler: it samples the *proposed* reduction (from the LLM `reduction_range`, a default spread, or — Discovery/UX, no proposal — the guardrail band) and **re-runs `effective_ai_reduction(...)` per draw**, so the clamp + codebase·seniority moderation + penalty nonlinearity hold on every draw. Returns constant 0 when the phase has no AI-tooling band.
...
- `app/settings/` — edits the AI-reduction guardrail bands **and** the team-scaling (Brooks's-Law + diminishing-returns) coefficients (gear icon) via `GET`/`PUT /admin/reduction-bands` and `GET`/`PUT /admin/staffing-coefficients`.
