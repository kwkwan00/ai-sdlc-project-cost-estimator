...
- **HTTP** (`routers/wbs.py`) — draft CRUD + duplicate + `preview`/commit (see the endpoint list above). `_to_storage` / `_from_storage` map between `WbsDraft` and the Neo4j row shape. Commit persists a `method="wbs"` envelope (Postgres history + `save_estimate_envelope` + `save_wbs_tree`) and retires the draft.
- **Frontend** (`frontend/app/wbs/{,new,team,edit/[draftId]}`) — landing+resume, describe, a team transition page (prefills roster + **awaits** tooling classification before drafting so AI savings aren't baked in as all-`none`), and the editor (`<WbsTreeViewEditor>` MUI X Tree View + the contingency input + debounced autosave + Re-evaluate/Submit). The shared review renders WBS via `<WbsTreePanel>` and hides twin-only widgets.
