...
`hours = raw_screen_points × DSF × ICM × IF`, then the system applies the responsive modifier (×1.15) when `is_responsive` is true.
...
- `iteration_factor` (IF, 1.0–2.5) — 1.0 (2 rounds agile), 1.3 (3 rounds mixed), 1.4 (4-5 rounds non-tech), 1.5 (5+ regulated)

### Responsive

- `is_responsive` — boolean. Multi-platform mobile/web responsive design; the system adds +15% effort when true.
...
> SCP → 127 × 0.7 × 1.3 × 1.0 ≈ 116 hrs, then ×1.15 responsive ≈ 133 hrs most-likely.
...
- **Size:** for your least-certain size driver — here the continuous `iteration_factor` (design rounds) — give `iteration_factor_range: {low, high}` (the ~80%-confidence interval; your point value is the mode), OR `estimate_cov` (0–0.6, the coefficient of variation). If you give neither, the system derives a band from `confidence`.
